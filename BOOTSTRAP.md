# BOOTSTRAP.md — Setup Instructions for stock-analysis skill

## What's this for:
The following instructions are to produce data and scripts in order to fulfill the stock-analysis. The scripts should be there already. but in case we have a cold start then we will need to start from scratch.  Check with the human why they didn't get the script from Github rather than re-creating the scripts here first.  Once confirm, you can proceed based on the human feedback.
